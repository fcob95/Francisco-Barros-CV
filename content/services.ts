...
/**
 * Service catalog (5 services), ported by value from
 * `design-assets/v1/services/services.data.ts`. The SEO-primary positioning:
 * applied AI for business, with pricing/revenue analytics as the differentiator
 * (last entry).
 *
 * Each service is indexable copy (problem → what's included → outcome). `slug`
 * doubles as the section anchor id and the Service JSON-LD @id fragment, kept
 * as authored. Copy is placeholder-grounded in Francisco's real CV skills —
 * safe to ship; no invented client names or metrics beyond the CV.
 */
